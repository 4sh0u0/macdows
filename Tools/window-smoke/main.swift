...
enum EdgeProfile {
    ...
    /// readable` is the OTHER half of `RemoteWindow.edgeProfileSample(currentMapped:)`'s `nil`: a surface is
    ...
    ///
    /// `stale-surface` is the 2026-09-15 addition (E-D (e3)) and is about the READER, not about
    /// the window: a surface IS displayed and IS readable, but it is not the surface the
    /// registry's current mapping describes, so its four ratios would answer a question nobody
    /// asked. That run's About window presented one frame and then took two `.surfaceMapped`
    /// events with no frame behind them; the finish sample re-profiled the first surface and
    /// printed a line byte-identical to first-frame while `[f1]` reported the newer mapping.
    /// Refusing, and naming both mapped sizes, is the only honest line for that state.
    ...
    static func line(
        windowId: UInt32, sample: RemoteWindow.EdgeProfileSample?, hasDisplayedContent: Bool,
        isVisible: Bool, at phase: Phase
    ) -> String {
        guard case .profile(let profile)? = sample else {
            // The window's own `hasDisplayedContent` is what separates the two `nil` reasons: the
            // profile function returns one `nil` and does not (and should not) carry a reason
            // across the registry boundary. A `.stale` sample carries its own reason and its own
            // two sizes, so `hasDisplayedContent` has nothing to say about it.
            if case .stale(let edgeMapped, let currentMapped)? = sample {
                return unavailableLine(
                    windowId: windowId, reason: .staleSurface,
                    // The two sizes ARE the line's content: `edge-mapped=` is the mapping the
                    // displayed surface was presented under, `current-mapped=` the one the
                    // registry holds now (the same lookup `[f1]` reads). Both are the DECLARED
                    // GFX sizes, not a profiled sub-rect, so they are formatted the way `[f1]`
                    // formats its own -- integral values as integers, anything else at three
                    // decimals -- rather than truncated the way a profiled `mapped=` is.
                    detail: " edge-mapped=\(fmtSize(edgeMapped)) current-mapped=\(fmtSize(currentMapped))",
                    isVisible: isVisible, at: phase
                )
            }
            let reason: Unavailable = hasDisplayedContent ? .surfaceNotReadable : .noDisplayedSurface
            return unavailableLine(
                windowId: windowId, reason: reason, detail: "", isVisible: isVisible, at: phase
            )
        }
    ...
    /// The `unavailable=` grammar, in ONE place, so the three reasons cannot drift apart: the
    /// token, then whatever that token needs to be readable (`detail`, empty for two of them),
    /// then `visible=` and `at=`.
    ///
    /// `visible=` only on this shape: a profiled line identifies its window through `mapped=`, and
    /// these have no profile numbers at all. Since every window in the registry gets a line, a
    /// reader needs some way to tell the dialog under study from a 1x1 bookkeeping window that
    /// will never draw anything. (The RULE for finding the window under study is still id-matching
    /// against `[f1]`; this is the cheap first cut.)
    private static func unavailableLine(
        windowId: UInt32, reason: Unavailable, detail: String, isVisible: Bool, at phase: Phase
    ) -> String {
        "[edge] id=\(windowId) unavailable=\(reason.rawValue)\(detail)"
            + " visible=\(isVisible ? 1 : 0) at=\(phase.rawValue)"
    }

    /// How many frames this window has PRESENTED, as its own line right after that window's
    /// `[edge]` line (O-A finish, 2026-09-15 E-D (e3)). MEASUREMENT ONLY, like everything else in
    /// this enum -- no verdict, no exit code.
    ///
    /// WHY IT IS A SEPARATE LINE. It is a property of the WINDOW's whole history, not of the
    /// surface any one `[edge]` line profiled, and the three `[edge]` shapes already print
    /// different field sets; appending it to each of them would make every existing shape's text
    /// change, and the 2026-09-15 record and the pre-registration both quote those verbatim. Read
    /// together with `[edge] unavailable=stale-surface`, `count=1` says the server published no
    /// frame for the newer mapping at all, while a larger count says frames did arrive and the
    /// staleness is this client's.
    static func presentsLine(windowId: UInt32, count: Int, at phase: Phase) -> String {
    ...
    /// A DECLARED GFX size, formatted BY `F1BackingVsMapped.Observation.fmt` itself -- not by a
    /// second copy of its integral-as-integer / otherwise-`%.3f` rule (gate r1 m-2), because the
    /// whole purpose of this formatter is that the `edge-mapped=` / `current-mapped=` pair can be
    /// compared with an `[f1]` line character for character, and two copies of a rule drift. It
    /// does NOT truncate: a fractional mapped size is a fact about the server, and hiding it here
    /// is exactly how the profiled `mapped=` and this one would stop being about the same thing.
    static func fmtSize(_ size: CGSize) -> String {
        "\(F1BackingVsMapped.Observation.fmt(Double(size.width)))"
            + "x\(F1BackingVsMapped.Observation.fmt(Double(size.height)))"
    }
...
enum WindowSmokeGateSelfTest {
    static func run() -> Bool {
        ...
        // --- MoveResizeTarget.lock, About path: newest About wins (same rule as AboutTarget.pick) ---
        let leftoverAbout = MoveResizeTarget.Candidate(windowId: 5, title: "About Windows", firstSeenElapsed: 0.4)
        let ownAbout = MoveResizeTarget.Candidate(windowId: 900, title: "关于 Windows", firstSeenElapsed: 2.1)
        expect(
            MoveResizeTarget.lock(candidates: [leftoverAbout, ownAbout], filter: nil, preExisting: [5, 900])?.windowId == 900
                && MoveResizeTarget.lock(candidates: [ownAbout, leftoverAbout], filter: "", preExisting: [])?.windowId == 900
                // an explicit filter keeps its own order -- lowest id among the non-pre-existing, first-seen ignored
                && MoveResizeTarget.lock(
                    candidates: [
                        MoveResizeTarget.Candidate(windowId: 20, title: "无标题 - Notepad", firstSeenElapsed: 9),
                        MoveResizeTarget.Candidate(windowId: 30, title: "Untitled - Notepad", firstSeenElapsed: 1)
                    ], filter: "Notepad", preExisting: []
                )?.windowId == 20,
            "moveResizeLockAboutPathPrefersTheNewestAbout: with no filter the About first seen latest is locked even when a leftover About has the lower id; an explicit filter still takes the lowest non-pre-existing id"
        )
        // --- the About heuristic prefers THIS run's About over a leftover one (STATUS pool, lock-r1 I-4: the
        // lowest-id About won, so an About left on the host by an earlier run beat the one this session's
        // winver just created; memo :101 records such a leftover with a run-to-run constant id) ---
        // Rule: newest by first WindowCreate wins; same first-seen time -> lowest id (the old order).
        expect(
            AboutTarget.pick(candidates: [
                AboutTarget.Candidate(windowId: 328362, firstSeenElapsed: 0.4),   // leftover, enumerated at connect
                AboutTarget.Candidate(windowId: 510198942, firstSeenElapsed: 2.1) // this run's winver
            ])?.windowId == 510198942
                && AboutTarget.pick(candidates: [
                    AboutTarget.Candidate(windowId: 510198942, firstSeenElapsed: 2.1),
                    AboutTarget.Candidate(windowId: 328362, firstSeenElapsed: 0.4)
                ])?.windowId == 510198942
                // a newer About with a LOWER id still wins (it is time, not id, that decides)
                && AboutTarget.pick(candidates: [
                    AboutTarget.Candidate(windowId: 900, firstSeenElapsed: 0.4),
                    AboutTarget.Candidate(windowId: 100, firstSeenElapsed: 2.1)
                ])?.windowId == 100
                // ties fall back to the historical lowest-id order
                && AboutTarget.pick(candidates: [
                    AboutTarget.Candidate(windowId: 900, firstSeenElapsed: 1.0),
                    AboutTarget.Candidate(windowId: 100, firstSeenElapsed: 1.0)
                ])?.windowId == 100
                && AboutTarget.pick(candidates: [AboutTarget.Candidate(windowId: 7, firstSeenElapsed: 3.0)])?.windowId == 7
                && AboutTarget.pick(candidates: []) == nil,
        ...
        expect(
            AboutTarget.describe(AboutTarget.Choice(windowId: 510198942, firstSeenElapsed: 2.1, tiedAtChosenTime: 1, runnerUpFirstSeen: 0.4), of: 2)
                ...
                && AboutTarget.describe(AboutTarget.Choice(windowId: 3, firstSeenElapsed: 2.1, tiedAtChosenTime: 1, runnerUpFirstSeen: -1), of: 3)
                == "newest of 3 candidates by first WindowCreate (windowId=3 first seen @2.100s; runner-up never observed)"
                && AboutTarget.describe(AboutTarget.Choice(windowId: 7, firstSeenElapsed: 3.0, tiedAtChosenTime: 1, runnerUpFirstSeen: nil), of: 1)
                == "the only candidate (windowId=7 first seen @3.000s)"
                && AboutTarget.describe(AboutTarget.Choice(windowId: 7, firstSeenElapsed: -1, tiedAtChosenTime: 1, runnerUpFirstSeen: nil), of: 1)
                ...
                && AboutTarget.describe(AboutTarget.Choice(windowId: 100, firstSeenElapsed: 1.0, tiedAtChosenTime: 2, runnerUpFirstSeen: 1.0), of: 2)
                == "lowest id of 2 candidates (all tied at first-seen @1.000s; newest-wins did not decide)"
                && AboutTarget.describe(AboutTarget.Choice(windowId: 100, firstSeenElapsed: -1, tiedAtChosenTime: 3, runnerUpFirstSeen: -1), of: 3)
                == "lowest id of 3 candidates (none observed at a first WindowCreate; newest-wins did not decide)"
                && AboutTarget.describe(AboutTarget.Choice(windowId: 2, firstSeenElapsed: 5.0, tiedAtChosenTime: 2, runnerUpFirstSeen: 5.0), of: 3)
                == "lowest id among the 2 candidates tied at the newest first-seen @5.000s (of 3; newest-wins eliminated 1)"
                // review about-target-r3 m-5: not pick-reachable, but describe is total -- no branch prints -1.000s
                && AboutTarget.describe(AboutTarget.Choice(windowId: 7, firstSeenElapsed: -1, tiedAtChosenTime: 1, runnerUpFirstSeen: 2.0), of: 2)
                == "newest of 2 candidates by first WindowCreate (windowId=7; first WindowCreate never observed; runner-up @2.000s)",
        ...
        expect(
            AboutTarget.pick(candidates: [AboutTarget.Candidate(windowId: 900, firstSeenElapsed: 0.4), AboutTarget.Candidate(windowId: 100, firstSeenElapsed: 2.1)])
                == AboutTarget.Choice(windowId: 100, firstSeenElapsed: 2.1, tiedAtChosenTime: 1, runnerUpFirstSeen: 0.4)
                ...
                == AboutTarget.Choice(windowId: 100, firstSeenElapsed: 1.0, tiedAtChosenTime: 2, runnerUpFirstSeen: 1.0)
                ...
                == AboutTarget.Choice(windowId: 3, firstSeenElapsed: 2.1, tiedAtChosenTime: 1, runnerUpFirstSeen: -1)
        ...
        // --- review about-target-r3 I-1: the Activate experiment WANTS the stale window (its doc: a background window
        // the server never painted for us), so the shared selection can be told to prefer the OLDEST first-seen ---
        expect(
            AboutTarget.pick(candidates: [AboutTarget.Candidate(windowId: 328362, firstSeenElapsed: 0.4), AboutTarget.Candidate(windowId: 510198942, firstSeenElapsed: 2.1)], prefer: .oldest)
                == AboutTarget.Choice(windowId: 328362, firstSeenElapsed: 0.4, tiedAtChosenTime: 1, runnerUpFirstSeen: 2.1)
                && AboutTarget.pick(candidates: [AboutTarget.Candidate(windowId: 9, firstSeenElapsed: 1.0), AboutTarget.Candidate(windowId: 4, firstSeenElapsed: 1.0)], prefer: .oldest)
                == AboutTarget.Choice(windowId: 4, firstSeenElapsed: 1.0, tiedAtChosenTime: 2, runnerUpFirstSeen: 1.0)
                && AboutTarget.describe(AboutTarget.Choice(windowId: 328362, firstSeenElapsed: 0.4, tiedAtChosenTime: 1, runnerUpFirstSeen: 2.1), of: 2, prefer: .oldest)
                == "oldest of 2 candidates by first WindowCreate (windowId=328362 first seen @0.400s; runner-up @2.100s)"
        ...
        struct Snap: Equatable { let windowId: UInt32; let title: String }
        var seen = FirstSeenClock()
        seen.observe(kind: .windowCreate, windowId: 328362, at: 0.4)
        seen.observe(kind: .windowCreate, windowId: 510198942, at: 2.1)
        let snaps = [Snap(windowId: 328362, title: "About Windows"), Snap(windowId: 510198942, title: "关于 Windows")]
        let newest = AboutTarget.choose(among: snaps, id: \.windowId, firstSeen: seen)
        let oldest = AboutTarget.choose(among: snaps, id: \.windowId, firstSeen: seen, prefer: .oldest)
        expect(
            newest?.window == snaps[1]
                && newest?.reason == "newest of 2 candidates by first WindowCreate (windowId=510198942 first seen @2.100s; runner-up @0.400s)"
                && oldest?.window == snaps[0]
                && oldest?.reason == "oldest of 2 candidates by first WindowCreate (windowId=328362 first seen @0.400s; runner-up @2.100s)"
                && AboutTarget.choose(among: [Snap](), id: \.windowId, firstSeen: seen) == nil,
            "chooseTargetReturnsTheChosenWindowAndItsReason: the generic selection every lock site calls hands back the window whose id pick chose -- not the first in input order -- with a reason that counts every candidate; no candidates -> nil"
        )
        ...
        var clock = FirstSeenClock()
        clock.observe(kind: .windowCreate, windowId: 328362, at: 0.4)
        clock.observe(kind: .windowCreate, windowId: 328362, at: 5.0) // a re-sent WindowCreate must NOT move the leftover forward
        clock.observe(kind: .windowCreate, windowId: 510198942, at: 2.1)
        expect(
            clock.elapsed(of: 328362) == 0.4 && clock.elapsed(of: 510198942) == 2.1 && clock.elapsed(of: 7) == -1
                && clock.candidates(for: [7, 510198942, 328362]) == [
                    AboutTarget.Candidate(windowId: 7, firstSeenElapsed: -1),
                    AboutTarget.Candidate(windowId: 510198942, firstSeenElapsed: 2.1),
                    AboutTarget.Candidate(windowId: 328362, firstSeenElapsed: 0.4)
                ]
        ...
        // --- owner ruling 2026-09-05 (closes review about-target-r4 I-3): "first WindowCreate never observed" is
        // OLDEST by definition -- it pre-dates this run's drain -- not an unknown to be excluded ---
        let unobserved = [AboutTarget.Candidate(windowId: 1, firstSeenElapsed: 0.4), AboutTarget.Candidate(windowId: 2, firstSeenElapsed: -1), AboutTarget.Candidate(windowId: 3, firstSeenElapsed: 2.0)]
        expect(
            AboutTarget.pick(candidates: unobserved, prefer: .oldest)
                == AboutTarget.Choice(windowId: 2, firstSeenElapsed: -1, tiedAtChosenTime: 1, runnerUpFirstSeen: 0.4)
                && AboutTarget.describe(AboutTarget.Choice(windowId: 2, firstSeenElapsed: -1, tiedAtChosenTime: 1, runnerUpFirstSeen: 0.4), of: 3, prefer: .oldest)
                == "oldest of 3 candidates by first WindowCreate (windowId=2; first WindowCreate never observed; runner-up @0.400s)"
                && AboutTarget.pick(candidates: unobserved, prefer: .newest)?.windowId == 3
                && AboutTarget.pick(candidates: [AboutTarget.Candidate(windowId: 2, firstSeenElapsed: -1)], prefer: .newest)
                == AboutTarget.Choice(windowId: 2, firstSeenElapsed: -1, tiedAtChosenTime: 1, runnerUpFirstSeen: nil)
                && AboutTarget.describe(AboutTarget.Choice(windowId: 2, firstSeenElapsed: -1, tiedAtChosenTime: 1, runnerUpFirstSeen: nil), of: 1, prefer: .newest)
        ...
        expect(
            MoveResizeTarget.reason(candidates: [leftoverAbout, ownAbout], filter: nil)
                ...
                && MoveResizeTarget.reason(candidates: [], filter: nil) == nil,
            "moveResizeLockSaysWhyOnTheAboutPath: the reason exists only for the About heuristic (the explicit-filter path is lowest-id plus pre-existing exclusion, stated by its own miss line)"
        )

        // --- the W4b Activate experiment: each checkpoint (t=6 s pre, t=20 s post) reports EXACTLY ONCE
        // (C second step r3, 2026-09-02: the post-sample came back nil because the About target had already
        // been closed, and the `activatePostRatio == nil` re-arm printed "n/a" on every tick -- 779 lines;
        // earlier floods: c-longrun-r1 3001 post lines, c2-run4 401, click 375 pre + 101 post) ---
        var cpValue = ActivateExperimentCheckpoint(threshold: 20)
        var valueSamples = 0
        let cpValueLines: [String?] = [19.9, 20.0, 20.5, 40.0].map { t in
            cpValue.run(elapsed: t, sample: { valueSamples += 1; return 0.98 }, targetVisible: { true })
        }
        var cpGone = ActivateExperimentCheckpoint(threshold: 20)
        let cpGoneLines: [String?] = [20.0, 21.0].map { t in cpGone.run(elapsed: t, sample: { nil }, targetVisible: { false }) }
        var cpBlank = ActivateExperimentCheckpoint(threshold: 20)
        var blankSamples = 0
        let cpBlankLines: [String?] = [20.0, 21.0, 21.9, 22.0, 30.0].map { t in
            cpBlank.run(elapsed: t, sample: { blankSamples += 1; return nil }, targetVisible: { true })
        }
        expect(
            cpValueLines == [nil, "98.0%", nil, nil] && valueSamples == 1,
            "activateCheckpointReportsAValueOnceAtItsThreshold: nothing before the threshold (the sampler is not even called), the value at the threshold, then silence -- one sample, one line"
        )
        expect(
            cpGoneLines == ["n/a (target no longer visible -- closed or unmapped before this checkpoint)", nil],
            "activateCheckpointSettlesImmediatelyWhenTheTargetIsGone: a nil sample on an invisible target is final at the threshold and prints once with that reason"
        )
        expect(
        ...
        expect(
            GeometryRounds.next(completedRounds: 1, total: 1) == .close
                ...
                // total < 1 is clamped to 1 (the knob's default), not "run forever" -- (0, 0) is the one input
                // that tells a clamped `max(1, total)` from a bare `total` (review georounds-r1 I-2)
                && GeometryRounds.next(completedRounds: 1, total: 0) == .close
                && GeometryRounds.next(completedRounds: 0, total: 0) == .again
        ...
        expect(
            EdgeProfile.line(
                windowId: 7, sample: .profile(edgeFull), hasDisplayedContent: true, isVisible: true,
                at: .firstFrame
            )
                ...
                && EdgeProfile.line(
                    windowId: 1,
                    sample: .profile(RemoteWindow.EdgeBorderProfile(
        ...
        let edgeInsetLine = EdgeProfile.line(
            windowId: 3, sample: .profile(edgeBand(firstDarkColumnFromLeft: 5)),
            hasDisplayedContent: true, isVisible: true, at: .finish
        )
        let edgeNoLeftFrameLine = EdgeProfile.line(
            windowId: 3, sample: .profile(edgeBand(firstDarkColumnFromLeft: nil)),
            hasDisplayedContent: true, isVisible: true, at: .finish
        )
        expect(
        ...
        expect(
            EdgeProfile.line(windowId: 7, sample: nil, hasDisplayedContent: false, isVisible: true, at: .finish)
                == "[edge] id=7 unavailable=no-displayed-surface visible=1 at=finish"
                && EdgeProfile.line(windowId: 9, sample: nil, hasDisplayedContent: false, isVisible: false, at: .firstFrame)
                ...
                // of edgeProfileSample(currentMapped:)'s nil (empty sub-rect / refused lock / non-32bpp), and it
                ...
                && EdgeProfile.line(windowId: 8, sample: nil, hasDisplayedContent: true, isVisible: true, at: .finish)
        ...
        // O-A finish (2026-09-15 E-D (e3)): the third shape, in the run's own numbers. The
        // displayed surface was presented under mapped 522x515; by finish the registry's current
        // mapping was 500x505 and no frame had been presented for it. The old code profiled the
        // first surface again and printed a line byte-identical to its first-frame line.
        let edgeStaleLine = EdgeProfile.line(
            windowId: 7,
            sample: .stale(
                edgeMapped: CGSize(width: 522, height: 515),
                currentMapped: CGSize(width: 500, height: 505)
            ),
            hasDisplayedContent: true, isVisible: true, at: .finish
        )
        expect(
            edgeStaleLine
                == "[edge] id=7 unavailable=stale-surface edge-mapped=522x515 current-mapped=500x505"
                    + " visible=1 at=finish"
                // It carries NO profile numbers: the whole point is that the four ratios would be
                // about a surface the reader is not asking about.
                && !edgeStaleLine.contains(" top=") && !edgeStaleLine.contains(" corners=")
                && !edgeStaleLine.contains(" firstDark=") && !edgeStaleLine.contains(" alloc="),
            "edgeProfileRefusesToProfileASurfaceTheRegistryHasMappedPast: a displayed surface whose mapped size is no longer the registry's current one prints unavailable=stale-surface with BOTH sizes (edge-mapped= as presented, current-mapped= as the registry holds now) and no profile numbers at all -- the 2026-09-15 About case, where the old line was byte-identical to first-frame while [f1] reported the newer mapping"
        )

        // gate r1 m-2: the stale shape's sizes and an `[f1]` line must be comparable character for
        // character, so the two are asserted to render identically -- integral (the only shape the
        // lab has ever produced) AND fractional (where a truncating copy of the rule would differ
        // silently, printing 514 for 514.5 and making the two `mapped=` values look equal).
        expect(
            EdgeProfile.fmtSize(CGSize(width: 522, height: 515)) == "522x515"
                && EdgeProfile.fmtSize(CGSize(width: 522, height: 514.5)) == "522x514.500"
                && EdgeProfile.fmtSize(CGSize(width: 522, height: 514.5))
                    == F1BackingVsMapped.Observation.fmt(522) + "x" + F1BackingVsMapped.Observation.fmt(514.5),
            "edgeStaleSizesUseTheSameFormatterAsTheF1Line: edge-mapped=/current-mapped= are rendered by F1BackingVsMapped.Observation.fmt itself -- integral values as integers, anything else at three decimals -- so a declared size can be compared with an [f1] line verbatim and a fractional one is never truncated into looking like a different measurement"
        )

        expect(
            EdgeProfile.presentsLine(windowId: 7, count: 1, at: .finish) == "[edge-presents] id=7 count=1 at=finish"
...
let geometryRounds = GeometryRounds.parseRounds(ProcessInfo.processInfo.environment["WINDOW_SMOKE_GEOMETRY_ROUNDS"])
...
/// in from each edge the first frame-like row/column sits (`RemoteWindow.edgeProfileSample(currentMapped:)`,
...
/// One pixel-sample checkpoint of the W4b Activate experiment (t=6 s pre, t=20 s post), as a value type that
/// owns its own once-only rule: `run` returns the line to print exactly once -- the value at the threshold,
/// or, for a nil sample, the reason (target gone: immediately; target visible but no sampleable backing:
/// after re-sampling for `grace` seconds). C second step r3 (2026-09-02): the old `activatePostRatio == nil`
/// re-arm printed "n/a" on every tick until finish (779 lines; earlier floods c-longrun-r1 3001, c2-run4 401,
/// click 375 pre + 101 post). Because the once-only rule lives in the type, a call site cannot forget to
...
struct ActivateExperimentCheckpoint {
    ...
    mutating func run(elapsed: TimeInterval, sample: () -> Double?, targetVisible: () -> Bool) -> String? {
        guard !settled, elapsed >= threshold else { return nil }
        let value = sample()
        if let value {
            settled = true; ratio = value
            return String(format: "%.1f%%", value * 100)
        }
        if !targetVisible() {
            settled = true
            return "n/a (target no longer visible -- closed or unmapped before this checkpoint)"
        }
        if elapsed >= threshold + grace {
            settled = true
            return String(format: "n/a (no pixel sample within %.1f s of the checkpoint -- target visible but no sampleable backing)", grace)
        }
        return nil
    }
}
...
/// Which of several title-matched windows a scenario should take when the host still shows one
/// left over from an earlier run (the RemoteApp session keeps windows across our connects, and the
...
enum AboutTarget {
    ...
    /// The winner, how many candidates share its first-seen time (`tiedAtChosenTime`, 1 = time decided,
    ...
    struct Choice: Equatable {
        let windowId: UInt32
        let firstSeenElapsed: TimeInterval
        let tiedAtChosenTime: Int
        let runnerUpFirstSeen: TimeInterval?
        var decidedByFirstSeen: Bool { tiedAtChosenTime == 1 }
    }

    /// Which end of the first-seen order wins; the lowest-id tie-break is the same either way.
    enum Preference { case newest, oldest }

    static func pick(candidates: [Candidate], prefer: Preference = .newest) -> Choice? {
    ...
    static func describe(_ choice: Choice, of count: Int, prefer: Preference = .newest) -> String {
        ...
        if choice.decidedByFirstSeen {
            ...
            return "\(word) of \(count) candidates by first WindowCreate (windowId=\(choice.windowId)\(seen)\(runnerUp))"
        }
    ...
    /// "returns the window `pick` chose, with a reason over every candidate" cannot silently become
    /// "returns the first in input order"). `matches` = the caller's own predicate already applied.
    static func choose<S>(
        among matches: [S], id: (S) -> UInt32, firstSeen: FirstSeenClock, prefer: Preference = .newest
    ) -> (window: S, reason: String)? {
        let candidates = firstSeen.candidates(for: matches.map(id))
        guard let choice = pick(candidates: candidates, prefer: prefer),
              let window = matches.first(where: { id($0) == choice.windowId }) else { return nil }
        return (window, describe(choice, of: candidates.count, prefer: prefer))
    }
}
...
/// its own create (review about-target-r2 m-4) -- which also means a server that destroyed and
/// re-created a leftover window would present it as NEW, and newest-wins would take it (r3 m-7; RAIL
/// destroys rather than unmap-remaps, and the registry too treats that window as new). Entries are not
/// reset between `WINDOW_SMOKE_CYCLES` cycles -- the registry is reused too -- so a window that survives
/// a cycle keeps its stamp and is rightly the older one when the next cycle's About appears beside it.
/// `observe(kind:windowId:at:)` is the only way in (`record` is private): `drainNow()` calls it per
/// window event, the self-test drives it with scripted sequences.
struct FirstSeenClock {
    ...
    private mutating func record(_ windowId: UInt32, at elapsed: TimeInterval) {
    ...
    /// was a `WindowUpdate` (the registry constructs it; `observe` ignores updates by design). That
    /// reading is a DEFINITION, not a fallback (owner ruling 2026-09-05, closing r4 I-3's open call):
    /// a window this run's drain never saw created was already there before its first visible order,
    ...
    func candidates(for windowIds: [UInt32]) -> [AboutTarget.Candidate] {
        windowIds.map { AboutTarget.Candidate(windowId: $0, firstSeenElapsed: elapsed(of: $0)) }
    }
}
...
enum MoveResizeTarget {
    ...
    /// every tick). Under an explicit filter: lowest `windowId` first, so a filter matching several
    /// windows does not depend on `windowSnapshots()`' dictionary order between runs. On the About
    /// path: `AboutTarget.pick`'s order (newest first-seen, then lowest id) -- see below.
    ///
    ...
    /// itself pre-existing at that moment, and the target that path has always meant. It orders by
    /// `AboutTarget.pick` instead -- the About first seen LATEST wins (this run's own winver; a
    /// leftover About from an earlier run was enumerated at connect, so it is older), equal
    /// first-seen falling back to the lowest id. Before this the About path took the lowest id
    /// outright, so a leftover About with the lower id beat this run's own (STATUS pool item from
    /// review movetarget-lock-r1 I-4; memo :101 names such a leftover with a run-to-run constant id).
    static func lock(candidates: [Candidate], filter: String?, preExisting: Set<UInt32>) -> Candidate? {
        guard isExplicit(filter) else {
            guard let choice = AboutTarget.pick(candidates: aboutCandidates(candidates, filter: filter)) else { return nil }
            return candidates.first { $0.windowId == choice.windowId }
        }
        return candidates.sorted(by: { $0.windowId < $1.windowId }).first { candidate in
            matches(title: candidate.title, filter: filter) && !preExisting.contains(candidate.windowId)
        }
    }

    /// The About path's reason half for the `[move-resize] target locked` line (review about-target-r1
    /// m-4: that line belongs to the lock the original finding was about, and it alone said nothing).
    /// `nil` under an explicit filter -- that path's order is stated by its own miss line -- or when
    /// nothing matches.
    static func reason(candidates: [Candidate], filter: String?) -> String? {
        guard !isExplicit(filter) else { return nil }
        let abouts = aboutCandidates(candidates, filter: filter)
        guard let choice = AboutTarget.pick(candidates: abouts) else { return nil }
        return AboutTarget.describe(choice, of: abouts.count)
    }

    private static func aboutCandidates(_ candidates: [Candidate], filter: String?) -> [AboutTarget.Candidate] {
        candidates.filter { matches(title: $0.title, filter: filter) }
            .map { AboutTarget.Candidate(windowId: $0.windowId, firstSeenElapsed: $0.firstSeenElapsed) }
    }
...
final class WindowSmokeDelegate: NSObject, NSApplicationDelegate {
    ...
    /// O-A: windowIds that have already said `unavailable=no-displayed-surface` /
    /// `unavailable=surface-not-readable` in the FIRST-FRAME phase. A window
    ...
    /// not an early insert into the one above. Three lines per window per run is the ceiling (the
    /// third is the stale set below); the
    ...
    /// O-A finish (gate r1 I-1): windowIds that have already said `unavailable=stale-surface` in
    /// the FIRST-FRAME phase -- a THIRD set, for the third time the same reasoning applies. This
    /// one is about a TRANSIENT state: a window sitting between a `.surfaceMapped` and the frame
    /// behind it is stale for that instant only, and the first-frame sampler fires once per drain
    /// batch that saw ANY window's frame, so the window under study can be caught mid-remap by a
    /// batch that was about somebody else entirely. Routing that line into
    /// `edgeProfileFirstFrameSampled` would spend this window's single profiled first-frame slot
    /// on it and cost the run its first-frame baseline for the WHOLE run -- exactly the outcome
    ...
    /// `WindowGeometry.clientWindowMoveLeftBorder(forStyle:)`'s constants (MacdowsCore, F6 (a); until 2026-09-05 the
    /// registry's private `measuredClientWindowMoveLeftBorder`) into a second file, and
    ...
    // Re-affirmed 2026-09-02 (review about-target-r3 I-1): the About-target lane's newest-wins rule
    // does NOT apply here; among simultaneous first-tick candidates the OLDEST first-seen is taken
    // (`AboutTarget.Preference.oldest`) and the lock is never moved afterwards.
    private var activateExperimentWindowId: UInt32?
    private var activatePreRatio: Double? { activatePre.ratio }
    private var activatePostRatio: Double? { activatePost.ratio }
    /// The two once-only checkpoints (t=6 s pre, t=20 s post) -- see `ActivateExperimentCheckpoint`.
    private var activatePre = ActivateExperimentCheckpoint(threshold: 6)
    ...
    /// sampled, nothing is printed, and `RemoteWindow.edgeProfileSample(currentMapped:)` is never entered.
    ///
    ...
    private func sampleEdgeProfiles(registry: RemoteWindowRegistry, at phase: EdgeProfile.Phase) {
        ...
        for snapshot in registry.windowSnapshots().sorted(by: { $0.windowId < $1.windowId }) {
            ...
            // actually gets printed. Still the single `registry.edgeBorderProfile` call site.
            let sample = registry.edgeBorderProfile(windowId: snapshot.windowId)
            if phase == .firstFrame {
                // Deduped per window AND per state: at most one "nothing yet" line, at most one
                // "stale right now" line and, later, exactly one line for the frame that does
                // arrive. `finish` is not deduped at all -- it runs once.
                if case .stale? = sample {
                    // gate r1 I-1: its own set, so a window caught mid-remap keeps its profiled
                    // first-frame slot (see `edgeProfileFirstFrameStale`'s own doc comment).
                    guard !edgeProfileFirstFrameStale.contains(snapshot.windowId) else { continue }
                    edgeProfileFirstFrameStale.insert(snapshot.windowId)
                } else if snapshot.hasDisplayedContent {
            ...
            // Right after that window's own `[edge]` line, always -- the pair is what makes a
            // stale line diagnosable. `?? 0` cannot lie here: the snapshot this loop is iterating
            // came from the same registry an instant earlier on the same actor, so the window is
            // still there; the fallback exists because the forwarder is honestly optional, not
            // because absence is expected.
            print(EdgeProfile.presentsLine(
                windowId: snapshot.windowId,
    ...
    private func drainNow() {
        ...
        _ = session.drainEvents { [weak self] event in
            ...
            // WINDOW_SMOKE_POPUP.
            if event.kind == .windowCreate || event.kind == .windowDelete { // the only kinds observe(kind:) acts on
                let now = Date().timeIntervalSince(self.startTime)
                if event.kind == .windowCreate { self.windowCreateTimestamps[event.windowId] = now }
    ...
    /// `AboutTarget.choose` over this run's `firstSeen` -- the one call every About-anchored choice in
    /// this file makes (the maximize, popup, input-test and cycle-close locks and the final size/paint
    ...
    private func chooseTarget<S>(
        ...
        AboutTarget.choose(among: matches, id: id, firstSeen: firstSeen, prefer: prefer)
    }
    ...
    private func runActivateExperiment(elapsed: TimeInterval, session: CRSession, registry: RemoteWindowRegistry) {
        ...
        // Exempt from the newest-wins rule ON PURPOSE (review about-target-r3 I-1): this experiment
        ...
        // A candidate the clock never saw a WindowCreate for (first-seen -1) is the OLDEST by
        // definition (owner ruling 2026-09-05; `FirstSeenClock.elapsed(of:)`'s doc) -- exactly the
        // stale window this experiment wants, so it wins here whenever one is present.
        ...
        if activateExperimentWindowId == nil {
            ...
            if let picked = chooseTarget(among: matches, id: \.windowId, prefer: .oldest) {
                activateExperimentWindowId = picked.window.windowId
                print("[experiment] Activate-experiment target locked: windowId=\(picked.window.windowId) title=\"\(picked.window.title)\" -- \(picked.reason)")
            }
        }
        guard let windowId = activateExperimentWindowId else { return }

        if let line = activatePre.run(
            elapsed: elapsed,
            sample: { registry.nonWhitePixelRatio(windowId: windowId, inBottomFraction: 0.2, sampleCount: 100) },
            targetVisible: { registry.windowSnapshots().contains { $0.windowId == windowId && $0.isVisible } }
        ) {
            print("[experiment] pre-Activate bottom-20% non-white ratio for windowId=\(windowId): " + line)
        }
        ...
        if let line = activatePost.run(
            elapsed: elapsed,
            sample: { registry.nonWhitePixelRatio(windowId: windowId, inBottomFraction: 0.2, sampleCount: 100) },
            targetVisible: { registry.windowSnapshots().contains { $0.windowId == windowId && $0.isVisible } }
        ) {
            print("[experiment] post-Activate bottom-20% non-white ratio for windowId=\(windowId): " + line)
        }
    }
    ...
    private func runMaximizeScenario(session: CRSession, registry: RemoteWindowRegistry) {
        ...
        case .waitingForTarget:
            ...
            let abouts = registry.windowSnapshots().filter { snap in
                snap.isVisible && snap.hasDisplayedContent
                    && (snap.title.localizedCaseInsensitiveContains("about") || snap.title.contains("关于"))
            }
            guard let picked = chooseTarget(among: abouts, id: \.windowId) else { return }
            let w = picked.window
            maximizeTargetWindowId = w.windowId
            print("[maximize] target locked: windowId=\(w.windowId) title=\"\(w.title)\" -- \(picked.reason)")
        ...
        case .awaitingRestore(let windowId, let sentAt):
            ...
            if let restored, SizeBand.isRestoredWidth(restored.0) {
                ...
                // Restore only runs after the maximize leg grew, so this round's verdict is simply "restored".
                maximizeRoundResults.append(true)
                maximizeRoundsCompleted += 1
    ...
    private func startMoveLeg(session: CRSession, windowId: UInt32, window: NSWindow, title: String, round: Int, registry: RemoteWindowRegistry) {
        ...
        if round > 1, !offset.fits {
            // Round 1 sends the unchecked/unfitting default exactly as before (said so above). A later
    ...
    private func runMoveResizeScenario(session: CRSession, registry: RemoteWindowRegistry) {
        ...
        case .waitingForTarget:
            ...
            // dictionary-ordered. `MoveResizeTarget.lock` orders on its own (lowest id under an
            // explicit filter, `AboutTarget.pick` on the About path), so this sort only fixes the
            // "titles seen" wording of the miss line below between runs.
            ...
            // window appears (env-202609-14, review resize-live-r2 I-2 / r3 I-r3-1); on the About
            // path the About first seen latest wins (review movetarget-lock-r1 I-4 / about-target-r1).
            let lockCandidates = candidates.map {
                MoveResizeTarget.Candidate(windowId: $0.windowId, title: $0.title, firstSeenElapsed: firstSeen.elapsed(of: $0.windowId))
            }
            moveResizeLockReason = MoveResizeTarget.reason(candidates: lockCandidates, filter: moveResizeTargetFilter)
    ...
    private func runPopupScenario(session: CRSession, registry: RemoteWindowRegistry) {
        ...
        case .waitingForTarget:
            ...
            guard let picked = chooseTarget(among: abouts, id: \.windowId),
                  let window = registry.window(forWindowId: picked.window.windowId) else { return }
            let w = picked.window
            popupOwnerWindowId = w.windowId
            popupWindowIdsBeforeKeystroke = Set(registry.windowSnapshots().map(\.windowId))
            print("[popup] target locked: windowId=\(w.windowId) title=\"\(w.title)\" -- \(picked.reason)")
    ...
    private func runInputTest(elapsed: TimeInterval, registry: RemoteWindowRegistry) {
        ...
        if inputTestWindowId == nil, elapsed >= 5 {
            ...
            if let picked = chooseTarget(among: matches, id: \.windowId) {
                ...
                print("[input-test] target locked: windowId=\(w.windowId) title=\"\(w.title)\" mode=\(inputTestMode) -- \(picked.reason)")
            }
        }
    ...
    private func finish() {
        ...
        // `shutdownAndWait()` below so every profiled surface is unambiguously one this session
        // presented and still holds. It is NOT a guarantee that the surface is the one the
        // registry's current mapping describes: `.surfaceMapped` can move that mapping on with no
        // frame behind it, and the 2026-09-15 About window (E-D (e3)) spent its whole run
        // displaying its first surface while the registry had remapped twice. That case is now
        // reported, not printed as numbers -- `[edge] unavailable=stale-surface`, with
        // `[edge-presents]` saying how many frames ever arrived. No-op with the knob unset.
        ...
        // (review about-target-r2 I-1). The reason is printed so this site, too, leaves live
        // evidence of which About it judged and why (r3 I-2).
        let aboutAnchor = chooseTarget(among: visibleWindows.filter { w in
            w.title.localizedCaseInsensitiveContains("about") || w.title.contains("关于")
        }, id: \.windowId)
        if let aboutAnchor {
            print("[final] About anchor for the size/paint gate: windowId=\(aboutAnchor.window.windowId) -- \(aboutAnchor.reason)")
        }
        let aboutWindow = aboutAnchor?.window
        ...
        if geometryRounds > 1, maximizeScenarioEnabled || moveResizeScenarioEnabled {
            ...
            if maximizeScenarioEnabled {
                check(
                    ...
                    "maximize scenario: geometry rounds -- all \(geometryRounds) maximize->restore rounds ran and passed (\(maximizeVerdict.text))"
                )
            }
            if moveResizeScenarioEnabled {
                check(
                    moveResizeRoundResults.count == geometryRounds,
                    "move-resize scenario: geometry rounds -- all \(geometryRounds) move->resize rounds ran (completed \(moveResizeRoundResults.count)/\(geometryRounds); "
